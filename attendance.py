...
def save_unknown_face(img_color, unknown_dir=UNKNOWN_DIR):
    ensure_dir(unknown_dir)
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")
    path = os.path.join(unknown_dir, f"unknown_{ts}.jpg")
    cv2.imwrite(path, img_color)
    print(f"[!] Saved unknown face to {path}")

def run_live(model_dir="model", db_path="attendance.db", cam_index=0, confidence_threshold=70):
    labels_path = os.path.join(model_dir, "labels.pickle")
    if not os.path.exists(labels_path):
        raise RuntimeError(f"No labels file found at {labels_path}. Run train.py first.")
    ...
    while True:
        ...
        for (x,y,w,h) in faces:
            ...
            try:
                ...
                # In case of error from model
                label_id, confidence = None, 999
            if label_id is not None and confidence < confidence_threshold:
                ...
                color = (0,255,0)
                if inserted:
                    print(f"[+] Attendance marked for {name} at {datetime.datetime.now().strftime('%H:%M:%S')}")
            else:
                name = "Unknown"
                text = f"{name} ({round(confidence,1) if isinstance(confidence, (int,float)) else '---'})"
                color = (0,0,255)
                save_unknown_face(face_crop_color)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-dir", default="model", help="Directory where trainer.yml and labels.pickle are stored")
    ...
    parser.add_argument("--threshold", default=70, type=float, help="Confidence threshold (lower = stricter)")
